...
// //     // Mostrar mensaje "No tenemos agencias" solo si se ha seleccionado el distrito y NO hay agencias
...
// //         const mensaje = `Hola Kamas! Vengo de su sitio web y estoy interesado en adquirir *${producto.nombre}*.\n\n`
// //             + `Cantidad: ${quantity}\n`
...
// //             + (productShippingCost !== null ? ` (Costo: S/.${productShippingCost})` : "");
...
//     // Al hacer clic en un tipo de envío, se guarda tanto el tipo como su precio (este precio es el del shipping seleccionado)
//     const handleShippingSelection = (tipoEnvio, precio) => {
//         setSelectedShippingType(tipoEnvio);
...
//     // Calcular el precio del envío según la lógica:
//     // - Si el distrito tiene agencias: se usa el precio obtenido desde la agencia (si ya se seleccionaron agencia y sede).
//     // - Si NO tiene agencias: se revisa si el JSON tiene un tipo de envío que coincida con el producto; si existe, se usa ese precio.
//     //   En caso contrario, se usa el fallback (35 para "Envío preferente" y 70 para "Envío aplicado").
//     let productShippingCost = null;
//     if(selectedDistritoObj && selectedDistrito){
//         if(tieneAgencias){
//             if(selectedAgencia && selectedSede){
...
//             }
//         } else {
//             // Si no tiene agencias, primero revisamos si existe un tipo de envío en el JSON
//             const tiposDeEnvio = selectedDistritoObj?.['tipos-de-envio'];
//             const tipoCorrespondiente = tiposDeEnvio?.find(tipo => tipo['tipo-de-envio'] === producto['tipo-de-envio']);
//             if (tipoCorrespondiente) {
//                 productShippingCost = tipoCorrespondiente.precio || tipoCorrespondiente.costos || 0;
...
//     // Mostrar mensaje "No contamos con agencias recomendadas..." solo si se ha seleccionado el distrito,
//     // NO hay agencias, y la provincia no es Lima ni Callao
//     const noAgenciasMessage = (selectedDistrito && !tieneAgencias &&
//         (selectedProvincia.toLowerCase() !== "lima metropolitana" && selectedProvincia.toLowerCase() !== "provincia constitucional del callao"))
//         ? "No contamos con agencias recomendadas para el distrito seleccionado, sin embargo podemos ayudarte a encontrar la mejor opción."
//         : null;
  
//     // El enlace de WhatsApp usa el precio del tipo de envío seleccionado (almacenado en shippingCost)
//     const getWhatsAppLink = () => {
//         const numeroWhatsApp = "+51907057521";
//         const mensaje = `Hola Kamas! Vengo de su sitio web y estoy interesado en adquirir *${producto.nombre}*.\n\n`
//             + `Cantidad: ${quantity}\n`
...
//             + (selectedSede ? `Sede: ${selectedSede}\n` : "")
//             + `\nTipo de envío seleccionado: *${selectedShippingType || "No seleccionado"}*`
//             + (shippingCost !== null ? `Costo de envío: S/.${shippingCost})` : "");
...
const handleShopButtonClick = (e) => {
    // Si no se ha seleccionado un tipo de envío, prevenimos el envío y mostramos alerta
    e.preventDefault();
    alert("Por favor, selecciona un tipo de envío antes de continuar.");
};
