...
function PaginaDeCategoria() {
    ...
    // 🔹 Capturar parámetros de la URL y aplicarlos como filtros al cargar la página
    useEffect(() => {
        const filtrosIniciales = {};
        
        searchParams.forEach((value, key) => {
            filtrosIniciales[key] = new Set([value]); // Guarda el filtro en formato Set
        });
                                ...
                                {filtros.map((filtro) => (
                                    <div className='filter' key={`filtro-${filtro.nombre}`}>
                                        ...
                                        <ul>
                                            {filtro.lista.map((opcion) => (
                                                <li key={`opcion-${filtro.nombre}-${opcion.nombre}`}>
                                                    <input type='checkbox' id={`filtro-${filtro.nombre}-${opcion.nombre}`} onChange={() => handleFiltroChange(filtro.nombre, opcion.nombre)}/>
                                                    <label htmlFor={`filtro-${filtro.nombre}-${opcion.nombre}`}>{opcion.nombre}</label>
                                                </li>
                                            ))}
                                        </ul>
